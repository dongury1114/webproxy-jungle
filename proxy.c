...
int main(int argc, char **argv) {
  ...
  if (argc != 2) {
    ...
    fprintf(stderr, "usage: %s <port> \n", argv[0]); //stderr는 버퍼 없이도 바로 출력 가능
  ...
  listenfd = Open_listenfd(argv[1]); // 3번 식별자를 만듬 argv[1]: 5000
  while (1) {
    clientlen = sizeof(clientaddr);
    connfd = Accept(listenfd, (SA *)&clientaddr, &clientlen); // Accept는 오류가 발생했을때 오류를 포함시켜서 나타내줌
...
void *thread(void *vargsp) {
  ...
  Pthread_detach(pthread_self()); //쓰레드가 종료되면 쓰레드가 사용하고 있던 자원을 반환 ( 모든 메모리 자원을 삭제 ) // 이걸 써두면 쓰레드가 종료되었을 때 알아서 실행
  doit(connfd);
  Close(connfd);
}
...
void doit(int connfd) {
  ...
  char endserver_http_header[MAXLINE]; //tiny server
  ...
  //프록시가 end server에 요청할 때 필요한 헤더를 만드는 과정
...
void build_http_header(char *http_header, char *hostname, char *path, int port, rio_t *client_rio) {
  ...
  while (Rio_readlineb(client_rio, buf, MAXLINE) > 0) {
    ...
    if (strncasecmp 3w sax(buf, connection_key, strlen(connection_key))
    ...
    *pos2 = '\0';
    ...
    if (pos2 != NULL) {
      ...
      *pos2 = '/';
      ...
      scanf(pos, "%s", hostname); //port를 입력하지 않으면 defualt인 80으로 동작
    }
  }
  return;
}
