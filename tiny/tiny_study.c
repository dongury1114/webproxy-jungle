...
void doit(int fd)
{
  ...
  Rio_readlineb(&rio, buf, MAXLINE);  // 그리고 rio(==connfd)에 있는 string 한 줄(응답 라인)을 모두 buf로 옮긴다. fd에서 들어온것을 buf에 저장, 따라서 connect fd에 들어오는 값이 buf에 들어감
