...
void *thread(void *vargs) { 
  ...
  doit(connfd);
  Close(connfd); 
  // connfd를 여러개로 만드는 이유? main 함수 while 돌 때마다 accept 쓰레드생성함수 호출, 
  // 생성 함수에서 쓰레드함수 호출하는데 호출할 때마다 connfd가 연결
  // 요청받으면 쓰레드 만들고, 이 쓰레드마다 connfd를 만든다. 그러면 프로세스는 하나인데 쓰레드 여러개 - 다중 쓰레드 개념..?
